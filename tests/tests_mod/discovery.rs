use std::fs;
use crate::common::{run_rmg, stdout_str, stderr_str, setup_git_repos};
...
fn no_print_no_projects_suppresses_message() {
    let tmp = tempfile::TempDir::new().unwrap();
    let output = run_rmg(tmp.path(), &["--no-print-no-projects", "list-projects"]);
    assert!(output.status.success());
    let stderr = stderr_str(&output);
...
fn glob_flag_overrides_default() {
    ...
    let output = run_rmg(tmp.path(), &["--glob", "vis*", "list-projects"]);
    assert!(output.status.success());
    let stdout = stdout_str(&output);
...
fn folders_flag_selects_specific() {
    ...
    let output = run_rmg(tmp.path(), &["--folders", &folders_arg, "list-projects"]);
    assert!(output.status.success());
    let stdout = stdout_str(&output);
