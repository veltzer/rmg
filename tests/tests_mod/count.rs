...
fn untracked_detects_new_files() {
    ...
    let output = run_rmg(tmp.path(), &["--stats", "count", "untracked"]);
    assert!(output.status.success());
    let stdout = stdout_str(&output);
...
fn count_dirty_terse_suppresses_names() {
    ...
    let output = run_rmg(tmp.path(), &["--terse", "--stats", "count", "untracked"]);
    assert!(output.status.success());
    let stdout = stdout_str(&output);
...
fn print_not_inverts_selection() {
    ...
    let output = run_rmg(tmp.path(), &["--print-not", "count", "untracked"]);
    assert!(output.status.success());
    let stdout = stdout_str(&output);
